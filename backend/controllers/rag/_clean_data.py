...
import unstructured.cleaners.core as cleaners
...
def clean_data_unstructured(texts):
    text = []
    for _text in texts:
        _text = cleaners.clean_ligatures(_text)
        _text = cleaners.replace_unicode_quotes(_text)
        _text = cleaners.clean_extra_whitespace(_text)
        _text = validate_and_fix_braces(_text)
...
def replace_multiple_dots(text):
    # Thay thế tất cả các chuỗi '..' liên tiếp thành một dấu '.'
