...
export function Dark(actionfunction: any) {
  const theme: any = store.getState().main;
  actionfunction({
    ...theme,
    class: "dark",
...
export const primaryColor1 = (actionfunction: any) => {
  ...
  actionfunction({
    ...theme,
    colorPrimaryRgb: "0, 49, 128",
    colorPrimary: "0 49 128",
  });
  localStorage.setItem("primaryRGB", "0, 49, 128");
  localStorage.setItem("primaryRGB1", "0 49 128");
};
...
export const LocalStorageBackup = (actionfunction: any) => {
  ...
  if (localStorage.bodyBgRGB) {
    let updateddarkBg = `${Number(localStorage.bodyBgRGB.split(" ")[0]) + 14} ${
      Number(localStorage.bodyBgRGB.split(" ")[1]) + 14
    } ${Number(localStorage.bodyBgRGB.split(" ")[2]) + 14}`;
    const theme = store.getState().main;
    actionfunction({
      ...theme,
